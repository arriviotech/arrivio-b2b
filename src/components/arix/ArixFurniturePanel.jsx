...
const ArixFurniturePanel = ({ design, onToggle, onAddAll, onShowAll }) => {
  const selectedIds = (design?.selectedItems || []).map(i => i.id);
  const totalSumOfAllItems = FURNITURE_ITEMS.reduce((sum, item) => sum + (item.price || 0), 0);
  const isAllSelected = FURNITURE_ITEMS.every(item => selectedIds.includes(item.id));
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between pt-1 border-b border-gray-100 pb-3">
        ...
        <span className="rounded-full bg-[#0f4c3a]/10 px-2.5 py-0.5 text-[11px] font-semibold text-[#0f4c3a]">
          {selectedIds.length} / {FURNITURE_ITEMS.length} items
        </span>
      </div>
...
export default ArixFurniturePanel;
