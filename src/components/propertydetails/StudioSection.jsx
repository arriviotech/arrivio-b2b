    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 rounded-xl bg-[#0f4c3a]/5 flex items-center justify-center">
          <User className="text-[#0f4c3a]" size={20} />
        </div>
        <div className="flex-1">
          <h2 className="text-xl font-heading font-bold text-[#111827]">Studio Apartments</h2>
          <p className="text-[12px] text-[#9ca3af]">{units.length} unit{units.length !== 1 ? 's' : ''} available</p>
        </div>
        {/* From price */}
        {units.length > 0 && (() => {
          const allPrices = units.flatMap(u => (u.unit_pricing_rules || []).map(p => p.monthly_rent_cents));
          const cheapest = allPrices.length > 0 ? Math.min(...allPrices) / 100 : property.price || 0;
          return (
            <div className="text-right shrink-0">
              <span className="text-[10px] text-[#9ca3af] font-medium">from </span>
              <span className="font-bold text-lg text-[#111827]">€{cheapest.toLocaleString()}</span>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-[#e5e7eb] overflow-hidden">
        {/* Table header */}
        <div className="flex items-center px-5 py-2 bg-[#f7f7f7] text-[10px] font-bold uppercase tracking-widest text-[#9ca3af] border-l-4 border-l-[#0f4c3a]">
        ...
        {units.map((unit, i) => {
          ...
          const price = activePricing ? activePricing.monthly_rent_cents / 100 : property.price || 0;
